 ...
 * ===============
 *
 * Structures, or *structs*, are composite objects made up of labeled fields. Each field can have
 * its own type, which may be any type supported by FXSL including other composite types and
 * wildcards. Structs form the counterpart to {@link ArrayType arrays}, which are homogeneously
 * typed lists, while structs are heterogeneous maps. The field labels and types define a struct;
 * two struct declarations with the exact same labeled types, regardless of order, are considered
 * equal. Thus, if two structs have the same field but have different types, the structs are
 * different. Similarly, two structs with the same typed field are not equal if the label is not the
 * same.
 *
 * @author Michael Ludwig
 */
@Immutable
public final class StructType extends EfficientEqualityBase implements Type {
  ...
  /**
   * Get every field defined by this type. The keys are the field names and the corresponding map
   * values are the field types. The returned map cannot be modified.
   *
